...
- (void)_initialize
{
	...
	_alignment = JKPageControlAlignmentCenter;
	_verticalAlignment = JKPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
	...
	CGFloat left = 0.0f;
	switch (_alignment) {
		case JKPageControlAlignmentCenter:
			left = CGRectGetMidX(rect) - (size.width / 2.0f);
			break;
		case JKPageControlAlignmentRight:
...
- (CGFloat)_topOffsetForHeight:(CGFloat)height rect:(CGRect)rect
{
	...
	switch (_verticalAlignment) {
		case JKPageControlVerticalAlignmentMiddle:
			top = CGRectGetMidY(rect) - (height / 2.0f);
			break;
		case JKPageControlVerticalAlignmentBottom:
